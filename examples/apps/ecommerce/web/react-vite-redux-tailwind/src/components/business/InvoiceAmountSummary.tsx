...
const InvoiceAmountSummary = (props: InvoiceAmountSummaryProps) => {
  ...
  return (
    <div className={`flex flex-col ${className}`}>
      <div
        className={`flex border-b border-grey justify-between pb-1 mb-2 ${isRTL ? "flex-row-reverse" : ""}`}
      >
      ...
      {total !== discountedTotal && (
        <div
          className={`flex border-b border-grey justify-between mb-2 ${isRTL ? "flex-row-reverse" : ""}`}
        >
      ...
      >
        ...
        <span className="font-semibold">
          {formatAmount(discountedTotal, currency)}
        </span>
      </div>
    </div>
  );
