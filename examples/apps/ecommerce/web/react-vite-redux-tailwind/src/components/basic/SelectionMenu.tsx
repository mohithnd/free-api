...
const SelectionMenu = (props: SelectionMenuProps) => {
  ...
  return (
    <div className="relative" ref={selectionMenuRef}>
      <button
        onClick={toggleMenu}
        className={`flex justify-between items-center w-full ${headingClassName}`}
      >
        <span>{heading}</span>
        {!isLG && (
          <UpArrow className={`w-4 h-4 ${!isMenuShown && "rotate-180"}`} />
        )}
      </button>
          ...
          {items.map((item, index) => (
            ...
            >
              ...
              >
                {item.icon && item.icon}
                <span
                  className={`whitespace-nowrap ${isRTL ? "mr-2" : "ml-2"}`}
                >
                  {t(item.textKey)}{" "}
                </span>
              </button>
